...
def create_layout(gui_settings):
    """
    ...
    """
    # assign the GUI settings to name p for ease of use
    p = gui_settings
    ...
    controls_column1 = [
        ...
        [
            ...
            sg.Spin(
                ...
                initial_value=p.default_contrast,
                font=("Helvetica", 20),
                key="contrast_slider",
                pad=(0, p.pad_y),
            ),
        ...
        [
            ...
            sg.Spin(
                [i for i in range(-100, 100)],
                initial_value=p.default_saturation,
                font=("Helvetica", 20),
        ...
        [
            ...
            sg.Spin(
                [i for i in range(0, 100)],
                initial_value=p.default_sharpness,
                font=("Helvetica", 20),
    ...
    controls_column2 = [
        [
            ...
            sg.Spin(
                ...
                initial_value=p.default_exposure,
                font=("Helvetica", 20),
                key="exposure_slider",
                pad=(0, p.pad_y),
            ),
        ...
        [
            ...
            sg.Spin(
                ...
                initial_value=p.default_image_no,
                font=("Helvetica", 20),
        ...
        [
            ...
            sg.Spin(
                [i for i in range(0, 100)],
                initial_value=p.default_time_step,
                font=("Helvetica", 20),
        ...
        [
            ...
            sg.Spin(
                [i for i in range(1, 100)],
                initial_value=p.default_vid_time,
                font=("Helvetica", 20),
    ...
    controls_column3 = [
        ...
        [
            ...
            sg.Button(
                "+ Resize +", size=(10, 1), font="Helvetica 14", pad=(p.pad_x, p.pad_y)
            ),
        ],
        [
            sg.Button(
        ...
        [
            ...
            sg.Button(
                "Where am I?", size=(10, 1), font="Helvetica 14", pad=(p.pad_x, p.pad_y)
            ),
        ],
    ...
    controls_column4 = [
        [
            sg.Text(
                "Grey scale:", font=("Helvetica", p.font_size, "bold"), pad=(0, p.pad_y)
            ),
            sg.Checkbox(
                "",
                size=(int(10), 1),
        ...
        [
            ...
            sg.Checkbox(
                "", size=(int(10), 1), enable_events=True, key="dark", pad=(0, p.pad_y)
            ),
            sg.Text(
                "light:", font=("Helvetica", p.font_size, "bold"), pad=(0, p.pad_y)
            ),
            sg.Checkbox(
                "", size=(int(10), 1), enable_events=True, key="light", pad=(0, p.pad_y)
            ),
        ],
        [
            sg.Text("bias:", font=("Helvetica", p.font_size, "bold"), pad=(0, p.pad_y)),
            sg.Checkbox(
                "", size=(int(10), 1), enable_events=True, key="bias", pad=(0, p.pad_y)
            ),
            sg.Text(
                "flat:   ", font=("Helvetica", p.font_size, "bold"), pad=(0, p.pad_y)
            ),
            sg.Checkbox(
                "", size=(int(10), 1), enable_events=True, key="flat", pad=(0, p.pad_y)
            ),
        ],
    ]
    ...
    with picamera.PiCamera(resolution=(3280, 2464)) as camera:
        ...
        while True:
            ...
            if event == "Save Location":
                cam_folder_save = sg.PopupGetFolder(
                    "save_folder",
                    initial_folder="{}".format(Parameters.default_save_folder),
                    no_window=True,
                    keep_on_top=True,
                )
            ...
            cam_brightness = int(
                values["brightness_slider"]
            )  # Grabs the user set brightness value
            cam_contrast = int(
                values["contrast_slider"]
            )  # Grabs the user set contrast value
            cam_saturation = int(
                values["saturation_slider"]
            )  # Grabs the user set saturation value
            cam_sharpness = int(
                values["sharpness_slider"]
            )  # Grabs the user set sharpness value
            cam_exposure = int(
                values["exposure_slider"]
            )  # Grabs the user set exposure value
            cam_no_images = int(
                values["no_images_slider"]
            )  # Grabs the user set number of images to be taken
            cam_time_step = int(
                values["time_step_slider"]
            )  # Grabs the user set time increment between images
            cam_vid_time = values[
                "video_duration_slider"
            ]  # Grabs the user set video length

            # change the camera settings for the preview
            camera.brightness = (
                cam_brightness  # brightness     min: 0   , max: 255 , increment:1
            )
            camera.contrast = (
                cam_contrast  # contrast       min: 0   , max: 255 , increment:1
            )
            camera.saturation = (
                cam_saturation  # saturation     min: 0   , max: 255 , increment:1
            )
            camera.sharpness = (
                cam_sharpness  # sharpness      min: 0   , max: 255 , increment:1
            )
            ...
            if event == "+ Resize +":
                ...
                width, height = [
                    int(num)
                    for num in (resolution_list[res_counter]).split()
                    if num.isdigit()
                ]
            ...
            if event == "Crosshair On":
                img = Image.open(
                    os.path.join(os.path.dirname(sys.argv[0]), "crosshair.png")
                ).convert("RGBA")
            ...
            if event == "Record":
                ...
                video_save_file_name = "{}/Video_{}_{}s.yuv".format(
                    vid_folder_save, current_day_time, cam_vid_time
                )
            ...
            # capture image
                ...
                if cam_exposure > 1:
                    ...
                    for i in range(cam_no_images):
                        ...
                        image_save_file_name = "{}/Image_{}_no-{}_LE_{}s{}.jpeg".format(
                            cam_folder_save,
                            current_day_time,
                            i,
                            cam_exposure,
                ...
                # triggers multiple exposures
                    ...
                    for i in range(cam_no_images):
                        ...
                        image_save_file_name = "{}/Image_{}_no-{}{}.jpeg".format(
                            cam_folder_save, current_day_time, i, file_append
                        )
            ...
            # reset the camera settings to the default values
                ...
                window.FindElement("brightness_slider").Update(
                    Parameters.default_brightness
                )
                window.FindElement("contrast_slider").Update(
                    Parameters.default_contrast
                )
                window.FindElement("saturation_slider").Update(
                    Parameters.default_saturation
                )
                window.FindElement("sharpness_slider").Update(
                    Parameters.default_sharpness
                )
                window.FindElement("exposure_slider").Update(
                    Parameters.default_exposure
                )
                window.FindElement("no_images_slider").Update(
                    Parameters.default_image_no
                )
                window.FindElement("time_step_slider").Update(
                    Parameters.default_time_step
                )
                window.FindElement("video_duration_slider").Update(
                    Parameters.default_vid_time
                )
